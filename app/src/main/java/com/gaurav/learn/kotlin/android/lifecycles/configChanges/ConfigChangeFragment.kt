...
class ConfigChangeFragment: Fragment() {
    ...
    /**
     * NOTE:
     *      Let's consider we do not have addTextChangedListener for edtNumber
     *      Hence comment the code from 39 to 44 and just rotate the screen
     *      Then observe the behavior of EditText after rotation as it will be able to restore its state as
     *      it has id associated with it. but if id is not there then it will not be able to restore its state.
     *      android:id="@+id/edtNumber" is required for state restoration.
     *      Most (all?) Standard Views can restore their state automatically after config changes.
     *      To enable automatic state restoration, Views must have IDs.
     */
